				...
				<div className='flex flex-col h-screen max-h-screen'>
					...
					<div className='flex flex-col items-start py-3 px-10 w-full bg-accent/5 primary-height overflow-auto scrollbar'>
						...
						<div className='flex items-center justify-between w-full'>
							<div className='flex gap-4 items-center'>
								...
								<div className='flex items-center gap-2'>
									...
									<p className='text-lg font-bold text-black'>@{forum?.response?.userId.username}</p>
								</div>
							</div>
						...
						<div className='grid grid-cols-12 mt-4'>
							{forum?.response?.joined.includes(user?._id as string) && <div key={currentPage + "forum_feed_page" + forumPosts?.totalPosts} className='max-h-[80vh] min-h-[80vh] w-full px-5 overflow-y-auto scrollbar col-span-8' id="home-feed-test">
								<InfiniteScroll
							...
							<div className='px-4 py-3 col-span-4 h-max sticky left-0'>
								<h3 className='text-xl font-bold'>About</h3>
								<p className='mt-3'>{forum?.response?.description}</p>
								<button type='button' onClick={() => setPostDialog(true)} className='button py-2 mt-3 h-max'>Create Post</button>
								{
									!forum?.response?.public && <div className='mt-5 flex flex-col gap-1'>
										<div className='flex items-center gap-3'>
											<CiLock className='text-accent stroke-2' />
